...
module.exports = {
    config: {
        ...
        countDown: 5,
        role: 0,
        shortDescription: " Your lover",
        longDescription: "my love",
        category: "LOVE",
        guide: {
            vi: "{pn} @tag ",
            en: "{pn} @tag "
    ...
    onStart: async function ({ message, args, event, api }) {
        ...
        if (mention.length == 0) return message.reply("Please mention your love...?");
        else if (mention.length == 1) {
            const one = event.senderID, two = mention[0];
            bal(one, two).then(ptth => { message.reply({ body: "𝐌𝐲 𝐋𝐨𝐯𝐞 </100%<❤️‍🩹", attachment: fs.createReadStream(ptth) }) })
        } else {
...
async function bal(one, two) {
    ...
    avtwo.circle();
    const pth = "abcd.jpg";
    const img = await jimp.read("https://drive.google.com/uc?export=view&id=1ReMwmSFPTz5Z4lHLAxFiC8YVuzYJNwKl");
